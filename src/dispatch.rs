...
use twilight_gateway::{Message, Shard, ShardState as ConnectionState};
    ...
    loop {
        ...
        if let Some(EventTypeInfo(event_name, _)) = event_type {
            ...
            if event_name == "READY" {
                ...
                // Clone before simd_json parse since it mutates the string in place.
                #[cfg(feature = "simd-json")]
                let mut ready: Ready =
                ...
                // Clear the guilds — they are sent separately as GUILD_CREATE events
                ...
                // Relay dispatchable events to subscribed clients.
            ...
            // Update the guild cache for all dispatch events, including pre-READY
            // GUILD_CREATEs. Events not relevant to guild state are no-ops.
            shard_state.guilds.process_event(event_name, &payload, guild_id);
        }

        // Handle InvalidateSession (op 9): d field is true = resumable, false = not.
        if op.0 == 9 {
            let can_resume = payload.contains("\"d\":true");
